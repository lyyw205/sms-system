    ...
    with engine.begin() as conn:
        ...
        if "rooms" in inspector.get_table_names():
            ...
            if "is_hidden" not in existing_cols:
                conn.execute(text("ALTER TABLE rooms ADD COLUMN is_hidden BOOLEAN NOT NULL DEFAULT FALSE"))
                print("AUTO-MIGRATE: Added is_hidden column to rooms table")
        ...
        # daily_hosts: 언스/포차 매출 컬럼 (현금/카드/이체 각각 금액)
        if "daily_hosts" in inspector.get_table_names():
            ...
            for col in [
                ...
                if col not in cols:
                    conn.execute(text(f"ALTER TABLE daily_hosts ADD COLUMN {col} INTEGER"))
                    print(f"AUTO-MIGRATE: Added {col} column to daily_hosts table")
