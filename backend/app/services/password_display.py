...
def build_prefixed_password(room) -> str:
    ...
    if not digit_part.isdigit():
        # 9000ddf 수정 경로. 운영에서 이 분기가 찍히면 door_password 에
        # 숫자 외 문자 섞여 있다는 신호 — prefix 보안 효과 사라짐.
        diag("password_display.prefix_skipped", level="critical",
             room_id=getattr(room, 'id', None),
             reason="non_digit_base")
        return base
