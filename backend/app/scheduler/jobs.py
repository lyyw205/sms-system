...
def split_orphan_sweep_job():
    ...
    logger.info("[Scheduler] Split orphan sweep...")

    def _sweep(db, tenant):
        result = sweep_orphan_groups(db, today_kst())
        if result["groups_alerted"]:
            logger.warning(f"[Split Sweep] tenant={tenant.slug}: {result}")
