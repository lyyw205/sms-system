...
| # | 테스트 항목 | 검증 포인트 |
|---|-----------|-----------|
| FA-1 | 포인터 드래그 시작 | 그립 핸들 pointerDown → 드래그 시작 (HTML5 drag 아님) |
| FA-2 | 드래그 고스트 표시 | 이름 포함 pill이 커서 따라 실시간 이동 |
| FA-3 | 드롭 영역 하이라이트 | 방/미배정/파티/삭제 영역 드래그 시 배경색 변경 |
| FA-4 | 드롭 완료 → 배정 반영 | 방에 놓으면 배정 API 호출, 고스트 사라짐 |
...
| # | 테스트 항목 | 검증 포인트 |
|---|-----------|-----------|
| FC-1 | 연박 연결 모달 1단계 | 기준 예약자 선택 UI 표시, 날짜별 예약 목록 |
| FC-2 | 연박 연결 모달 2단계 | 추가 선택 또는 완료, 체인 미리보기 표시 |
...
| FD-9 | 드래그 중 메뉴 차단 | 드래그 진행 중 우클릭 → 메뉴 안 뜸 |
